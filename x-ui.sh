...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/AdNotFound/x-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/AdNotFound/x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
